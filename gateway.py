...
path_cleaned = '/Users/wan397/Desktop/IoT_and_Privacy/' + \
    date_object+'_gateway_location_.csv'


location_lib = np.array(pd.read_csv(fixed_devices_lib, usecols=[0, 3]))
for x in range(0, len(location_lib)):
    if(location_lib[x, 1]) == 'gateway':
        id_ = str(location_lib[x, 0])
        URL = head+id_+rear
        response = requests.get(URL)
        lot = []
        lon = []
        path_raw = '/Users/wan397/Desktop/IoT_and_Privacy/' + date_object+'_gateway'
        ...
        if (len(data) > 1):
            ...
            strings = id_+','+str(max_lot)+','+str(min_lot) + \
                ','+str(max_lon)+','+str(min_lon)
