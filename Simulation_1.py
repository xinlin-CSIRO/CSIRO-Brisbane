...
for x in range(0, 1000):
    lat_moving_tag=random.uniform(min_lat,max_lat) # independent test-->lat info
    lon_moving_tag=random.uniform(min_lon,max_lon) # independent test-->lon info
    ...
    for y in range (0, len (fix_devices)):    # relative distance calculation
       physical_distance_[y], rssi_[y]=rssi (moving_tag , fix_devices[y])   
    min_dis= ind = np.argmin(physical_distance_) #correct classification--> min relative distance
    max_rssi=ind = np.argmax(rssi_)  # guess-->max rssi
